...
def create_image(Red, Green, Blue, Alpha):
    # Take red, green, blue, and transparency channels [0, 1] and create an RGBA image
    ...
    Alpha_image = Image.fromarray(np.around(255*Alpha[::-1,:]).astype('uint8'))
...
def draw_annotations(image, wcs, name, ruler, compass, no_annotation_area):
    # Create an annotation area and draw a ruler, compass, and object name, if necessary
    if (name is not None or ruler is not None or compass) and not no_annotation_area:
...
def draw_name(image, name, loc=None):
    ...
    if loc is None:
        ...
        name_px = (loc, 30)
    draw.text((name_px[0], image.size[1]-name_px[1]), name, font=ImageFont.truetype("DejaVuSans.ttf", 24), fill="black", anchor="mm")
...
def draw_ruler(image, wcs, ruler):
    ...
    line_end_deg = line_start_deg.directional_offset_by(line_pos_ang, Angle(ruler[0]))
    ...
    draw.text((half_line[0], image.size[1]-half_line[1]-15), ruler[0], font=ImageFont.truetype("DejaVuSans.ttf", 24), fill="black", anchor="mm")
    draw.text((half_line[0], image.size[1]-half_line[1]+15), ruler[1], font=ImageFont.truetype("DejaVuSans.ttf", 24), fill="black", anchor="mm")
...
def mapping(value, cutoff, softening):
    ...
    mapped = scaling(value.astype(float), softening) / scaling(cutoff, softening)
...
def mark_sources(image, mask, wcs, source_list, offset):
    ...
    if source_list is not None:
        draw = ImageDraw.Draw(image)
        cat = Table.read(source_list) # anything tabular goes with columns RAdeg and DEdeg
...
def transform_colors(red, green, blue, cutoff, stretch, softening):
    ...
    if stretch == 0:
        ...
        # Place the background level at 0.5 and symmetrically stretch around this level
        posRed, posGreen, posBlue = map_colors(+red, +green, +blue, stretch, softening)
        negRed, negGreen, negBlue = map_colors(-red, -green, -blue, stretch, softening)
        Red, Green, Blue = 0.5 + np.mean([[posRed, posGreen, posBlue], [negRed, negGreen, negBlue]], axis=0)
